...
**Stacks tab** = BYO **connections** only (not agent create). Three primary sections/kinds:

| Kind | User adds | Card shows |
| --- | --- | --- |
| **Inference** | Bedrock (IAM vs API Key + Region) / OpenAI-compatible (Ollama, Groq, Zen, Custom) | Status, per-card verified models, configurable credentials / URLs |
| **Coding harness** | OpenCode (`oc-local`) / Cursor (later) | Status, registered models (`provider/model` pairs), candidate registration |
| **External agents** | MCP/A2A endpoint (stub) | Discovered external agents, status |
...
- **Local Models tab** (top-level nav): pull local models into volume (SSE progress, GPU status, flush, delete, 1-click **Enable as Inference**)  
