...
**Status:** Envelope + adapter + journal + gold + team OKF pack + extract + office Q&A + HITL + autonomy gate + **OKF export** = core v0 slices done. Shelf / import optional.

## Harness branch (OpenCode — design)

Same pack order; different adapter after pack. See [18_OPENCODE.md](./18_OPENCODE.md).

```text
Inference:  pack → stream_chat / gold tool loop → journal → extract(assistant text)
OpenCode A: pack → spawn serve(cwd=workspace) → session.chat + /event
            → token + thinking SSE → thinking.jsonl → journal
            → extract(assistant text only; never thinking)
```
