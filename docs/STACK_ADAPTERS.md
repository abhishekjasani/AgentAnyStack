...
**Harness may use Inference for models** when the product allows (e.g. OpenCode → Bedrock model allowlist). Office stores the link + BYO keys; **does not** reimplement the harness’s model router UI.
...
Add stack
  ...
  2) Product (by kind):
       Inference → Bedrock | Claude API | OpenAI | Ollama | …
       Harness   → OpenCode | Cursor | Claude Code | …
  3) Credentials / endpoint / region
  4) Harness only: optional “Models via” → pick an Inference connection
  5) Test → Save
```
...
| --- | --- |
| Inference · Bedrock | Keys/role, region, optional model allowlist |
| Inference · Ollama | Base URL |
| Inference · Claude API | API key |
| Harness · OpenCode | Reachability + link Inference for models |
| Harness · Cursor | API key / workers note |
...
│ Models via: Bedrock         │  │ Region · models allowed     │
│ Status: Connected ✓         │  │ Status: Connected ✓         │
│ Used by: 2 desks            │  │ Used by: OpenCode, 1 desk   │
│ [Edit] [Test]               │  │ [Edit] [Test]               │
└─────────────────────────────┘  └─────────────────────────────┘
```
...
| Status, linked models, used-by desks | Autonomy, pack depth |
| Product tiles (“Connect OpenCode”) | Label “adapter” in UI |
| Test / edit credentials | Full domain marketplace (Harvey, Miro, …) as live equals in v1 |

Models appear **inside** an Inference card (collapsed list), not as separate stack rows.

Roadmap tiles (“v0 · Soon”) OK; fake live integrations next to Cursor/OpenCode are not.

### Desk creation (elsewhere — Team)

```text
Team → Add agent
  → Runtime kind + engine (from Stacks connections)
  → models if harness linked Inference
  → workspace / pack / autonomy
  → writes agent.yaml
```

Desk badge example: `OpenCode · Bedrock` or `Ollama`.
